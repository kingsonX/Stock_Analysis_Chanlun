...
- `chanlun_app/review_service.py`：智能复盘服务，负责聚合龙虎榜、游资、涨跌停、连板与关注标的。
...
`GET /api/boards/search?q=&type=&source=dc|tdx|ths&limit=`

- 用于板块/概念搜索，支持三套来源：
  - `dc`：东方财富，走 `dc_index / dc_member`
  - `tdx`：通达信，走 `tdx_index / tdx_member`
  - `ths`：同花顺，走 `ths_index / ths_member`
- `type` 用于限定板块类型，不同来源的允许值由前端控件约束。
- 返回 `items`，至少包含 `ts_code/name/type/source`，用于智能选股页面做单独查询或组合筛选。
...
- `query_text` 已改为选填，不再是必填项；可以只使用板块源单独执行。
- 额外支持三套板块过滤参数：
  - `board_ts_code`、`board_name`、`board_type`
  - `tdx_board_ts_code`、`tdx_board_name`、`tdx_board_type`
  - `ths_board_ts_code`、`ths_board_name`、`ths_board_type`
...
`GET /api/review/overview?trade_date=`

- 返回智能复盘页聚合数据，按交易日汇总：
  - `dragon_tiger`：龙虎榜数据，来源 `top_list`
  - `hot_money_records`：游资原始明细，来源 `hm_detail`
  - `hot_money_trades`：按股票合并后的游资交易结果
  - `hot_money_state`：游资数据状态，可能为 `ok / rate_limited / error`
  - `hot_money_stats`：游资原始记录数与合并记录数
  - `limit_lists`：涨停 / 跌停 / 炸板分组，来源 `limit_list_d`
  - `ladder`：连板天梯，来源 `limit_step`
  - `focus_boards`：需要关注的板块，来源 `limit_cpt_list`
...
- 智能选股查询区采用“四来源独立卡片 + 公共参数”的布局：
  - 条件查询
  - 东方财富板块/概念
  - 通达信板块/概念
  - 同花顺板块/概念
  各来源都支持单独执行，也支持组合筛选；不要恢复成“条件必填”的旧交互。
- 智能复盘页采用后台工作台布局：左侧主菜单，主内容区显示统计卡与核心复盘表，右侧侧栏使用 `复盘要点 / 关注板块 / 关注股票` tab。
- 智能复盘页四张统计卡均为可点击卡片：
  - 龙虎榜：打开龙虎榜明细弹窗
  - 游资数据：打开游资明细弹窗
  - 涨停 / 跌停：打开涨停前排、炸板观察、跌停风险明细弹窗
  - 最高连板：打开连板天梯明细弹窗
- 复盘页内股票入口统一应可跳转到“股票分析”页面；若缺少标准 `ts_code`，允许回退到按股票名搜索。
- 复盘弹窗默认必须隐藏；样式层应保留 `.reviewModal[hidden] { display: none !important; }`，避免首次进入页面自动弹出。
